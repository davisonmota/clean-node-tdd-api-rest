...
const makeAuthUseCaseWhitError = () => {
  class AuthUseCaseSpy {
    async auth () {
      throw new Error('any_error_AuthUseCase.auth')
    }
  }
  return new AuthUseCaseSpy()
}
...
const makeEmailValidatorWithError = () => {
  class EmailValidatorSpy {
    isValid () {
      throw new Error('any_error_EmailValidator.isValid')
    }
  }
...
describe('Login Router', () => {
  ...
  test('Should return 500 if no httpRequest if provided', async () => {
    const { sut } = makeSut()
  ...
  test('Should return 401 when invalid credentials are provided', async () => {
    ...
    expect(httpResponse.body.error).toBe(new UnauthorizeError().message)
  })
  ...
  test('Should return 500 if no AuthUseCase is provided for LoginRouter', async () => {
    const sut = new LoginRouter()
    const httpRequest = {
      body: {
        email: 'any_email@mail.com',
  ...
  test('Should return 500 if AuthUseCase throws', async () => {
    const authUseCaseSpy = makeAuthUseCaseWhitError()
    const sut = new LoginRouter(authUseCaseSpy)
    const httpRequest = {
      body: {
        email: 'any_email@mail.com',
